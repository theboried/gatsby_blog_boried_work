...
${reservedFields.map(f => `  * "${f}"`).join(`\n`)}
  ...
  let layout = page.layout || null; // If no layout is set we try fallback to `/src/layouts/index`.
...
actions.createLayout = (layout, plugin, traceId) => {
  let id = layout.id || path.parse(layout.component).name; // Add a "machine" id as a universal ID to differentiate layout from
  // page components.
  ...
  const result = _joi.default.validate(internalLayout, joiSchemas.layoutSchema);

  if (result.error) {
    console.log(_chalk.default.blue.bgYellow(`The upserted layout didn't pass validation`));
    console.log(_chalk.default.bold.red(result.error));
